...
CanTxMsg Spe_Cmd = {0x7DF,0,CAN_ID_STD,CAN_RTR_DATA,8,0x02,0x01,0x0D,0x00,0x00,0x00,0x00,0x00};//请求车速
CanTxMsg Thr_Cmd = {0x7DF,0,CAN_ID_STD,CAN_RTR_DATA,8,0x02,0x01,0x11,0x00,0x00,0x00,0x00,0x00};//请求节气门开度
extern u16 EngineRPM;//每分钟发动机转数
extern u8  Speed;//车速 KM/h
extern u8  Throttle;//节气门开度 %
...
int main(void)
{
	...
	MainDisplay();
		...
		RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);	 //使能PB,PE端口时钟
			...
			GPIO_ResetBits(GPIOA,GPIO_Pin_8);						 //PB.5 输出高
	while(1)
			{
					if(con==1)
					{
								//con=0;
					Send_CANFrame(&Spe_Cmd);//向ECU请求数据 车速
					}
					if(con==2)
					{
								//con=0;
					...
					{
					con=0;
					Send_CANFrame(&Thr_Cmd);//向ECU请求数据 节气门开度
					}
					...
					if(RxFlay==1)//接收到正确返回数据 中断接收ECU返回的数据，在stm32f10x_it.c中的USB_LP_CAN1_RX0_IRQHandler函数
					{
								...
								Usart_SendByte(USART1,Speed);
								//Usart_SendByte(USART1,0xaa);
								Usart_SendByte(USART1,0x55);
						
					}		
					 MainDisplaynum(Speed,6);
					 MainDisplayzhuanshu(EngineRPM,2);
					MainDisplaynum(Throttle,9);
			}
}
