...
TWAI 是一种多主、多播、异步、串行通信协议。TWAI 还支持错误检测和信令以及内置消息优先级。

**多主控：**总线上的任何节点都可以发起消息的传输。

**组播：**当节点发送消息时，总线上的所有节点都会收到该消息（即广播），从而保证所有节点之间的数据一致性。然而，一些节点可以通过使用接受过滤（多播）选择性地选择接受哪些消息。

**异步：**总线不包含时钟信号。总线上的所有节点都以相同的比特率运行，并使用总线上传输的比特边缘进行同步。

**错误检测和信令：**每个节点都会持续监控总线。当任何节点检测到错误时，它将通过发送错误帧来发出检测到的信号。其他节点将接收错误帧并传输自己的错误帧作为响应。这将导致错误检测传播到总线上的所有节点。

**消息优先级：**消息包含 ID 字段。如果两个或多个节点尝试同时传输，则传输具有较低 ID 值的消息的节点将赢得总线仲裁。所有其他节点将成为接收器，确保任何时候最多有一个发送器。

### TWAI 消息

TWAI 消息分为数据帧和远程帧。数据帧用于向其他节点传递数据有效负载，而远程帧用于向其他节点请求数据帧（其他节点可以选择使用数据帧进行响应）。数据帧和远程帧有两种帧格式，称为**扩展帧**和**标准帧**，分别包含 29 位 ID 和 11 位 ID。TWAI消息由以下字段组成：

> - 29位或11位ID：确定消息的优先级（值越低优先级越高）。
> - 数据长度代码 (DLC) 介于 0 到 8 之间：指示数据帧的数据有效负载的大小（以字节为单位），或请求远程帧的数据量。
> - 数据帧最多包含 8 个字节的数据（应与 DLC 匹配）。

### 错误状态和计数器[](https://docs.espressif.com/projects/esp-idf/zh_CN/latest/esp32s3/api-reference/peripherals/twai.html#error-states-and-counters)

TWAI 协议实现了一种称为“故障限制”的功能，其中持续出错的节点最终将从总线中消除。这是通过要求每个节点维护两个内部错误计数器（称为**传输错误计数器（TEC）**和**接收错误计数器（REC））**来实现的。两个错误计数器根据一组规则递增和递减（其中计数器在出现错误时增加，在成功的消息传输/接收时减少）。计数器的值用于确定节点的**错误状态**，即**主动错误**、**被动错误**和**总线关闭**。

**Error Active：当****TEC和REC都小于128**时，节点处于Error Active状态，表明该节点运行正常。错误主动节点被允许参与总线通信，并且将通过在总线上自动传输**主动错误标志**来主动发出检测到任何错误的信号。

**被动错误：当****TEC 或 REC 大于或等于 128 时，**节点处于被动错误。错误被动节点仍然能够参与总线通信，但会在检测到错误时传输**被动错误标志。**

**总线关闭：当****TEC 大于或等于 256**时，节点变为总线关闭。总线关闭节点无法以任何方式影响总线（本质上与总线断开），从而将其自身从总线中消除。节点将保持总线关闭状态，直到它经历总线关闭恢复。

## [信号线和收发器](https://docs.espressif.com/projects/esp-idf/zh_CN/latest/esp32s3/api-reference/peripherals/twai.html#id15)[](https://docs.espressif.com/projects/esp-idf/zh_CN/latest/esp32s3/api-reference/peripherals/twai.html#signals-lines-and-transceiver)

TWAI 控制器不包含集成收发器。因此，要将 TWAI 控制器连接到 TWAI 总线，**需要外部收发器**。所使用的外部收发器的类型应取决于应用的物理层规范（例如，使用SN65HVD23x 收发器来实现ISO 11898-2 兼容性）。

TWAI 控制器的接口由 4 条信号线组成，分别为**TX、RX、BUS-OFF 和 CLKOUT**。这四根信号线可以通过 GPIO 矩阵路由到 ESP32-S3 的 GPIO 焊盘。

![img](https://image-1302263000.cos.ap-nanjing.myqcloud.com/img/blockdiag-f247078dba247fa28547521aeb4a91f2da61052e.png)

*TWAI控制器的信号线*[](https://docs.espressif.com/projects/esp-idf/zh_CN/latest/esp32s3/api-reference/peripherals/twai.html#id6)

**TX 和 RX：** TX 和 RX 信号线需要与外部收发器连接。两条信号线都将显性位表示/解释为低逻辑电平(0V)，将隐性位表示/解释为高逻辑电平(3.3V)。

**BUS-OFF：** BUS-OFF 信号线是**可选的**，每当 TWAI 控制器达到总线关闭状态时，BUS-OFF 信号线就会设置为低逻辑电平 (0V)。否则，BUS-OFF 信号线被设置为高逻辑电平（3.3V）。

**CLKOUT：** CLKOUT 信号线是**可选的**，输出控制器源时钟的预分频版本。
...
![image-20230728144808159](https://image-1302263000.cos.ap-nanjing.myqcloud.com/img/image-20230728144808159.png)

![image-20230728144825793](https://image-1302263000.cos.ap-nanjing.myqcloud.com/img/image-20230728144825793.png)

![image-20230728144837135](https://image-1302263000.cos.ap-nanjing.myqcloud.com/img/image-20230728144837135.png)

![image-20230728145136929](https://image-1302263000.cos.ap-nanjing.myqcloud.com/img/image-20230728145136929.png)
...
![image-20230728150228100](https://image-1302263000.cos.ap-nanjing.myqcloud.com/img/image-20230728150228100.png)

![image-20230728150235773](https://image-1302263000.cos.ap-nanjing.myqcloud.com/img/image-20230728150235773.png)

拓展帧

![image-20230728145838226](../../../.config/Typora/typora-user-images/image-20230728145838226.png)
