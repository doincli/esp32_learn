...
static esp_err_t i2c_master_sensor_test(i2c_port_t i2c_num, uint8_t *data_h, uint8_t *data_l)
{
    ...
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, CONFIG_BH1750_ADDR << 1 | WRITE_BIT, ACK_CHECK_EN);
    i2c_master_write_byte(cmd, 0x25, ACK_CHECK_EN);
    i2c_master_stop(cmd);
    ...
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, CONFIG_BH1750_ADDR << 1 | READ_BIT, ACK_CHECK_EN);
